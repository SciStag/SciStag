...
def test_web_fetch_and_cache(tmp_path):
    ...
    WebCache.cleanup()
    assert not response_details.get(
        FROM_CACHE, False
    )  # should have been removed by now due to timeout
    assert response_details.get(STORED_IN_CACHE, False)  # should still be in cache
    WebCache.flush()
    # store on disk
