...
def test_azure_file_sink_basics():
    ...
    assert (
        AzureStorageFileSink.setup_container(sink.service, "existing", create=True)
        is not None
    )
    assert (
        AzureStorageFileSink.setup_container(sink.service, "notexisting", create=False)
        is None
    )
    assert (
        AzureStorageFileSink.setup_container(
            sink.service, "existing", create=True, reuse_existing=False
        )
        is None
    )
...
def test_azure_file_sink_creation():
    ...
    fs = AzureStorageFileSink(target=sink_target_container_string + "/subFolder")
    assert fs.sub_folder == "subFolder/"
    fs = AzureStorageFileSink(target=sink_target_container_string + "/subFolder2/")
    ...
    fs = AzureStorageFileSink(
        service=sink_target_connection_string, container="testcontainer"
    )
    assert fs.container is not None
    fs = AzureStorageFileSink(
        service=sink_target_connection_string,
        container="testcontainer",
        create_container=False,
    )
    assert fs.container is not None
    with pytest.raises(ValueError):
        fs = AzureStorageFileSink(service=sink_target_connection_string, container=None)
    with pytest.raises(ValueError):
        AzureStorageFileSink(service=sink_target_connection_string, container=123)
    with pytest.raises(ValueError):
...
def test_azure_file_sink_upload():
    ...
    assert AzureStorageFileSink.upload_file(full_target + "/testfiled.bin", data=b"123")
    with pytest.raises(ValueError):  # no container name
        AzureStorageFileSink.upload_file(sink_target_connection_string, data=b"123")
...
@pytest.mark.skipif(skip_long_tests, reason="Long running Azure tests disabled")
def test_azure_file_sink_deletion():
    ...
    sink.store(random_name, verification_data)
    time.sleep(1.0)
    source = FileSource.from_source(sink_target_container_string, fetch_file_list=True)
