...
def paint(slide: Slide, canvas: Canvas):
    ...
    canvas.text((x, 70), text=text, color=(255, 0, 0), font=font,
                stroke_color=(0, 0, 0), stroke_width=2)
    emoji: Image = slide.cache_data("emoji",
                                    lambda: EmojiDb.render_emoji(":deer:",
                                                                 size=512))
    canvas.draw_image(emoji, (slide.width() // 2 - emoji.width // 2, 220))


if __name__ == "__main__":
    app = SimpleSlide.create_simple_app(title="Hello SciStag!", on_paint=paint,
                                        resolution=(700, 768))
    app.run_as_kivy_app()
